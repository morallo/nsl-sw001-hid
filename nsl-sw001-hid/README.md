# N-SL SW001 kernel driver (hid-nsl-sw001)

A dedicated custom Linux HID driver that makes the **N-SL SW001** (a cheap
Nintendo Switch "Pro Controller" clone, VID:PID=`057e:2009`) work over
**Bluetooth on a PC / Steam Deck**.

This is one of the two implementations in the project root (the other is the
HID-BPF filter in `../nsl-sw001-hid-bpf/`). See the top-level
`../README.md` for the shared background.

## What the driver does

- **only claims N-SL SW001 units** (matches `057e:2009` **plus** the `9C:54:00`
  Bluetooth OUI), so genuine Nintendo controllers and other `057e:2009` clones
  are left to the normal drivers;
...
- **reads the factory IMU calibration** from the controller's SPI flash
  (`0x6020`) at connect and applies it in `raw_event` (accel/gyro offsets and
  scales, e.g. gyro scale 15335), so IMU data is accurate instead of
  default-scaled;
- **implements rumble** as evdev `FF_RUMBLE` on the gamepad node, sent via the
  controller's dedicated rumble-only HID output report `0x10` (the rumble
  field of the standard report `0x01` is ignored by the SW001), sustained by
  a ~50ms kernel resend and stopped by a zero-amp countdown.

It also:
- **blacklists the broken `hid-nintendo` match** so the SW001 is not stolen
  from this driver.
  **⚠️⚠️⚠️THIS WILL PREVENT YOUR GENUINE NINTENDO CONTROLLERS FROM WORKING!!! ⚠️⚠️⚠️**
...
Bluetooth **OUI** (the first three bytes of the BD_ADDR). The two SW001 units
tested use the **unregistered IEEE OUI block `9C:54:00`** (each unit gets its
own MAC suffix, e.g. `9C:54:00:4B:8F:A7` or `9C:54:00:4C:C9:3B`).
Genuine Nintendo controllers use Nintendo's registered OUIs, so they (and other
`057e:2009` clones of different OUIs) are left alone and fall through to the
normal driver path.
...
This folder also contains small SDL 3 diagnostics tools (`enum_sdl3`,
...
- **IMU needs the udev rule:** the accelerometer/gyroscope are exposed as a
  ...
  install steps above.
- **Capture button** is not visible to Steam Input: SDL's built-in Switch Pro
  mapping has no `misc1:` binding.
- **A/B physical mapping:** because A/B are mapped to their physical positions
  (south/east), Xbox-native games display them swapped.
- **Rumble is generic, not HD-rumble:** the SW001's motors are driven with
  evdev `FF_RUMBLE` using the standard dekuNukem frequency/amplitude encoding
  (sine-approximation), not the Switch's HD-rumble waveforms. On Android the
  clone exposes no rumble at all (the motor blip on connect is its own
  power-on self-test).
